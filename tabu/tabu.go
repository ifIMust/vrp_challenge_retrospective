...
// Neighbors are generated by moving single loads to all other possible positions in the routes.
	...
	for i, sourceRoute := range route {
		...
		for n, targetRoute := range route {
			...
			for targetRouteIndex := 0; targetRouteIndex < len(targetRoute)+1; targetRouteIndex += 1 {
				...
				for sourceRouteIdx := 0; sourceRouteIdx < len(sourceRoute); sourceRouteIdx += 1 {
